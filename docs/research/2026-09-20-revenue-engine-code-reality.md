...
Reuse the evidence schemas, public-URL guards, deterministic audit, content-addressed identities, append-only D1 plans, owner read models, and fixture test helpers. Defer deeper artifact retention/projection, production Worker wiring, R2, Browser, live source/contact providers, Jev/OpenAI runtime, and CRM/reply unification until the minimal slice proves value. For M4, keep Cloudflare forwarding/manual owner replies and Resend outbound/reply reconciliation as separate typed boundaries; Cloudflare forwarding cannot itself send custom-domain replies, so prove the human-triggered Resend reply flow or a separately reviewed free owner-only send-as route. Retire or quarantine legacy autonomous intake only after shadow reconciliation and a 30-day rollback window; do not delete legacy tables or routes in this cycle. Keep legacy Gmail outreach available only behind existing explicit emergency/approval gates, never as the v2 target.
