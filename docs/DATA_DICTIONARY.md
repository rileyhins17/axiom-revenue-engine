...
| RevenuePrivateKwOwnerDecision | Inactive append-only schema-0069 ledger for one exact authenticated owner-dossier acceptance. It mirrors the complete proof/dossier/contact parent, stores only opaque authentication bindings plus the owner enum, rejects raw auth identifier keys and conflicting proof/binding identities, and requires future current-session recheck plus exact durable reload. No writer or reader exists yet, so a schema-valid row or copied JSON is not trusted durable proof. |
