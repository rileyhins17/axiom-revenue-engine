...
| CurrentWebsiteEvidenceEligibilityReceipt | Content-addressed proof that one exact website-evidence result was evaluated from fresh, in-process transaction-sealed completeness executions and current R2 HEAD receipts for every artifact manifest. Its schema alone is not trusted and it grants no phase, provider, outreach, or cost authority. |
| RevenueCurrentWebsiteEvidenceEligibilityReceipt | Append-only schema-0068 durable copy of the exact eligibility receipt, mirrored against its business, sealed workflow, terminal artifact set, completeness proofs, and R2 HEAD receipts. Every reload rechecks canonical JSON, row identity, and database-clock freshness; stale rows remain history and cannot become current proof. |
...
| PrivateKwCurrentWebsiteEvidenceProgressInput | Frozen, module-private normalized input for the `CURRENT_WEBSITE_EVIDENCE` progress phase. It can be derived only from the exact in-process `CURRENT` result of a `DURABLE_RELOAD` and rechecks the full ten-business scope, selected manifest record, source/workflow predecessor, website proof, durable eligibility receipt, and chronology. It cannot append progress, create a phase receipt/checkpoint, access a runtime or provider, mutate data, or authorize downstream work. |
| PrivateKwCurrentWebsiteEvidenceProgressAppend | Validation-only in-memory boundary that accepts only the exact module-private website-evidence phase input and its unchanged content-addressed parent checkpoint. It uses the canonical appender, verifies exactly one target receipt and no unrelated record change, freezes the result, and returns the same checkpoint instance on an exact retry. It has no operator, file, database, runtime, provider, real-data, deployment, outreach, or cost path. |
| RevenueLeadAssessmentD1DurableReload | Deeply frozen read-only trust result created only after D1 reloads the exact assessment receipt and complete immutable row set, verifies all eight schema-0061 writer guards, rebuilds the assessment from its sealed workflow source and business, and classifies freshness from the database clock. Only the exact in-process `CURRENT` object may feed assessment progress proof; copied JSON, stale history, or a writer response is untrusted. |
...
| PrivateKwContactReviewProgressInput | Deeply frozen, module-private normalized input for the `CONTACT_REVIEW` phase. It can be derived only from the exact in-process `CURRENT` durable contact reload and exact guarded assessment checkpoint; it regenerates the contact-review proof internally, rechecks the complete manifest business/predecessor/chronology, and privately binds itself to one manifest and unchanged parent. It cannot append progress, create a phase receipt/checkpoint, access files/runtime/providers, mutate data, contact anyone, or authorize execution. |
| PrivateKwContactReviewProgressAppend | Validation-only in-memory boundary that accepts only the exact module-private contact-review input and its unchanged content-addressed assessment parent. It uses the canonical receipt builder/appender, independently verifies one contact-review receipt, the expected summary transition, unchanged prior prefix and other nine businesses, freezes the result, and returns the same checkpoint instance on exact retry. It has no operator, file, database, runtime, provider, real-data, deployment, outreach, or cost path. |
| TrustedOwnerLeadDetailResponse | Deeply frozen exact object returned by the SELECT-only owner-dossier reader and registered in a private in-process trust set. Its complete schema remains serializable for viewing, but a copied or reconstructed object cannot satisfy the proof boundary. Current routing is projected separately from the immutable qualification snapshot. |
| PrivateKwOwnerDossierAcceptanceProof | Content-addressed validation-only proof binding the exact current trusted dossier digest, manifest/cohort/business/source identity, current website/qualification/contact-review lineage, exact four-receipt contact parent, and one explicit Riley/Aidan read-only declaration inside a five-minute window. It explicitly does not prove session authentication or durable decision storage and authorizes no receipt, append, mutation, provider, outreach, deployment, send, or spend. |
| PrivateKwOwnerDossierProgressInput | Deeply frozen, module-private normalized input for `OWNER_DOSSIER`. It regenerates acceptance proof internally from the exact trusted dossier and exact guarded contact checkpoint, has no supporting receipt beyond its content-addressed primary proof, and privately binds itself to the unchanged parent. It cannot append progress, persist an owner decision, authenticate a reviewer, access live resources, contact anyone, or authorize execution. |
| PrivateKwOwnerDossierProgressAppend | Validation-only in-memory boundary that accepts only the exact module-private owner-dossier input and unchanged content-addressed contact parent. It uses the canonical receipt builder/appender, independently verifies the final receipt, fully-completed count, next-incomplete cohort pointer, manifest-bound target identity, unchanged prior prefix and other nine businesses, freezes the result, and returns the same checkpoint instance on exact retry. It authenticates or persists no owner decision and has no operator, file, database, runtime, provider, real-data, deployment, outreach, send, or cost path. |
| PrivateKwAuthenticatedOwnerDecisionRecord | Deeply frozen content-addressed candidate derived from the exact in-process dossier proof, a current active Better Auth session with verified Riley/Aidan email, and server time. HMAC-SHA-256 binds subject, session, key version, proof, decision, owner, and time while raw user/session/email identifiers remain absent. The candidate is validation-only and grants no persistence, progress, provider, outreach, send, deployment, or cost authority. |
| RevenuePrivateKwOwnerDecision | Inactive append-only schema-0069 ledger for one exact authenticated owner-dossier acceptance. It mirrors the complete proof/dossier/contact parent, stores only opaque authentication bindings plus the owner enum, rejects raw auth identifier keys and conflicting proof/binding identities, and requires current-session recheck plus exact durable reload. No migration has been applied and no live writer exists; a schema-valid row or copied JSON is never trusted by itself. |
| PrivateKwAuthenticatedOwnerDecisionD1Result | Deeply frozen private result created only after the injected boundary verifies every schema-0069 writer guard, uses database time, verifies the versioned decision HMAC, and exactly reloads every mirrored column. `FRESH_COMMIT`, mutation-free `EXACT_REPLAY`, and historical `DURABLE_RELOAD` remain distinct. A durable reload does not prove a current session or authorize progress, runtime, providers, outreach, send, deployment, or cost. |
