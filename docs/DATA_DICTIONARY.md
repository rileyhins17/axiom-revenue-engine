...
| SuppressionEvent | Broader contact/recipient/business suppression and legacy reconciliation remain proposed M4 work; identity changes and imports must preserve stops. Do not infer this from the v2 status enum or the business-only stop below. |
| RevenueBusinessStopEvent | Migration 0072 and a typed D1 boundary implement one append-only manual owner stop for an exact v2 business. The authenticated dossier saves it and current list/detail routes show blocked or fail closed when stop state cannot be checked. This does not implement contact-level suppression, legacy reconciliation, consent, provider/send gates, or a resume action. See [ADR 0046](adr/0046-add-a-manual-business-stop-before-outreach.md). |
