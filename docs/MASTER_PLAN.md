...
Absence of provider access still leaves a complete offline product: existing evidence fixtures/imports, deterministic audit and qualification, owner dossiers, editable message templates, a manual task list, and owner-recorded outcomes. Real email requires an owner-approved lawful sender identity and an actual sending route, even when sent manually; an alias or DNS record does not manufacture that capability. The selected zero-paid-mailbox path is Cloudflare Email Routing from the root domain to each owner's existing destination inbox for inbound handling only. As of 2026-09-23, no provider is selected for unsolicited cold email: Resend's current Acceptable Use Policy bars unsolicited cold outreach and requires recipients to opt in, while Cloudflare Email Service currently says it is intended only for transactional email. Resend may be reconsidered only for opt-in or transactional mail if its current terms and account eligibility allow it; Cloudflare Email Sending is not a cold-outreach alternative. Public DNS confirms routing and provider-looking records exist but does not prove active forwarding, account ownership, verified sender, correct DKIM, or send readiness. ([Resend Acceptable Use Policy](https://resend.com/legal/acceptable-use), [Cloudflare Email Service FAQ](https://developers.cloudflare.com/email-service/reference/faq/))
...
Only verified email may eventually automate. Phone, form, and social are manual tasks under their own rules; manual does not mean exempt from consent, platform rules, calling restrictions, or suppression. Owner-led business-to-business calls are a candidate zero-new-provider first-touch route, but Axiom must verify its free National DNCL registration and operational telemarketing controls before calling; the business-consumer exemption from the list rules does not remove the registration or internal do-not-call duties. See the [first-touch route review](reviews/2026-09-23-first-touch-channel-route.md) and [CRTC guidance](https://crtc.gc.ca/eng/phone/telemarketing/biz.htm). No unattended AI calling integration.
...
An approval binds exact recipient, sender identity, inbound reply owner/destination, business, body/subject digest, source evidence versions, campaign/offer/variant, policy version, and expiry. Any material change invalidates it. Batch review may approve each visible exact message without requiring the owner to click through every internal persistence step. Current command-specific approval rules remain until a separately reviewed replacement exists.

### 5.7 Sending and replies

Sending is the last feature to activate. First prove Cloudflare destination ownership plus a documented manual monitoring process, bounce handling, unsubscribe suppression, owner assignment, and stop controls using owner-controlled mail sinks. No provider is currently selected for cold email: Resend's AUP explicitly prohibits unsolicited messages, including cold outreach, and Cloudflare Email Service is currently for transactional mail. Keep outbound prospecting to separately governed owner-managed manual calls or other permitted tasks; calls remain held until National DNCL registration and calling controls are verified. Resend can be evaluated only for opt-in or transactional messages if its current terms permit the exact use. Any future adapter must set an explicitly verified `From`, the Cloudflare-routed owner `Reply-To`, a stable `Message-ID`, and `In-Reply-To`/`References` for programmatic replies; persist provider IDs and reconcile webhook events idempotently. Cloudflare forwarding itself is inbound only. No automatic follow-ups. ([Resend Acceptable Use Policy](https://resend.com/legal/acceptable-use), [Cloudflare Email Service FAQ](https://developers.cloudflare.com/email-service/reference/faq/))
...
Treat provider acceptance, confirmed send, delivery evidence, bounce, human reply, qualified interest, and opportunity as different events. A successful API response is not proof the recipient saw a message. If a send times out after dispatch, record `UNKNOWN_OUTCOME`, stop retries, and reconcile against provider history or owner review. Application idempotency and a stable Message-ID do not create an exactly-once guarantee. Resend `email.bounced`, `email.complained`, `email.delivery_delayed`, `email.failed`, and `email.suppressed` events require durable, signature-checked, `svix-id`-deduplicated reconciliation and immediate suppression where applicable. Legacy Gmail OAuth/send/sync routes remain quarantined from v2 and are not proof of provider readiness. See [Resend send API](https://resend.com/docs/api-reference/emails/send-email), [Resend event types](https://resend.com/docs/webhooks/event-types), and [Resend webhooks](https://resend.com/docs/webhooks/introduction).
...
The total ceiling remains C$50/month, excluding the existing ChatGPT/Codex subscription. It covers actual allocated engine costs, including platform, domain allocation, source/verification/AI, tax and FX. No account, mailbox, paid commitment, or free-tier entitlement is presumed active. The target inbound route has zero paid mailbox seats: Cloudflare Email Routing forwards mail to existing owner inboxes. Resend Free's published price does not make it an allowed sender for unsolicited outreach; its AUP requires opt-in. Cloudflare Email Sending requires Workers Paid and is documented as transactional only. No cold-email route is selected until a provider's current policy and legal use are verified together. Opt-in/transactional Resend use is a separate conditional candidate, not a prospecting authorization. Incremental provider spend remains C$0 and no provider activation is authorized by this plan. Paid mailboxes and Workspace seats are out of scope unless Riley explicitly reverses the zero-paid-mailbox decision. ([Resend pricing](https://resend.com/pricing), [Resend Acceptable Use Policy](https://resend.com/legal/acceptable-use), [Cloudflare Email Service pricing](https://developers.cloudflare.com/email-service/platform/pricing/), [Cloudflare Email Service FAQ](https://developers.cloudflare.com/email-service/reference/faq/))

Use [the provider/budget plan](PROVIDER_AND_BUDGET_PLAN.md) as the rate-card and readiness source. The old Workspace seat line is out of scope; the plan models zero paid mailbox seats and records Resend's published free plan separately from actual account/quota entitlement. Taxes, FX, provider terms, retention, limits and account eligibility can change. Prepaid verification minimums can exceed the entire monthly ceiling even when their amortized unit cost looks cheap. Free credits require a real account and terms check and are never assumed renewable forever.
...
| M4 — Ready-to-contact workbench and reply readiness | Exact messages/manual tasks with contact/consent proof and a named reply owner | Cloudflare inbound destination/routing proof, manual reply monitoring, suppression and stop controls; outbound email provider and terms remain unselected for cold outreach; no live send |
| M5 — Controlled first-touch pilot | A small number of approved real conversations with complete history | Owner authorizes exact cohort/campaign and channel; proposed 5–10/week total applies only after channel-specific legal/provider approval; cold email stays off until a compatible provider and legal route are proven; no follow-ups |
...
## 11. Delegated decisions and external gates

The plan selects architecture and sequencing now. Riley delegated the ten-business identity review to Codex on 2026-09-23; each decision must retain its evidence and name Codex as reviewer rather than Riley or Aidan. This delegation does not by itself prove source-data rights, a working capture route, qualified website need, provider permission, or a release gate. Before external actions become real, verify actual provider/account commitments and taxes; approved sender/legal identification and mailing address; Cloudflare destination ownership/rules; a provider whose published terms allow the exact outbound channel and recipient-consent model; any Resend domain/key/webhook state for an explicitly opt-in or transactional use; source-data rights and exact research scope; each deployed release/migration; any production policy experiment; exact outreach campaign/cohort; and a budget increase if the measured setup cannot fit. Public DNS is evidence only: it cannot prove forwarding activation, account ownership, verified sender, valid DKIM, or provider permission for cold outreach.

Each external gate that still requires Riley must arrive as a prepared, understandable packet with consequences, price, next action and rollback. Do not ask the owner to solve routine implementation choices or to authorize already-authorized reversible work again. The 2026-09-20 planning revision was docs-only; Riley's later instructions authorize continued local implementation and delegated business review, while deployment, migration, paid activation, and live outreach still use their separate recorded gates.
