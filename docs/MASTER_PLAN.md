...
The total ceiling remains C$50/month, excluding the existing ChatGPT/Codex subscription. It covers actual allocated engine costs, including platform, domain allocation, source/verification/AI, tax and FX. No account, mailbox, paid commitment, or free-tier entitlement is presumed active. The target first route has zero paid mailbox seats: Cloudflare Email Routing forwards inbound mail to existing owner inboxes, while Resend Free is a conditional outbound option whose current published limits and account eligibility require owner verification. A paid mailbox is a fallback decision if reply ownership or privacy cannot be met by the zero-cost route.

Use [the provider/budget plan](PROVIDER_AND_BUDGET_PLAN.md) as the rate-card and readiness source. The old Workspace seat line is no longer the default architecture; the plan now models zero paid mailbox seats and records Resend's published free plan separately from actual account/quota entitlement. Taxes, FX, provider terms, retention, limits and account eligibility can change. Prepaid verification minimums can exceed the entire monthly ceiling even when their amortized unit cost looks cheap. Free credits require a real account and terms check and are never assumed renewable forever.
