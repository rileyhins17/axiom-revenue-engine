...
Absence of provider access still leaves a complete offline product: existing evidence fixtures/imports, deterministic audit and qualification, owner dossiers, editable message templates, a manual task list, and owner-recorded outcomes. Real email requires an owner-approved lawful sender identity and an actual sending route, even when sent manually; an alias or DNS record does not manufacture that capability. The current zero-paid-mailbox target is Cloudflare Email Routing from the root domain to each owner's existing destination inbox for inbound handling, with a typed Resend outbound/reply adapter after its account, key, verified-domain, legal, webhook, and suppression gates pass. Public DNS observed on 2026-09-21 supports Cloudflare root MX/SPF, DMARC quarantine, and a Resend verification token, but does not prove active forwarding destinations, provider account/key access, sender verification, complete DKIM, or send readiness.
...
Sending is the last feature to activate. First prove Cloudflare destination ownership plus a documented manual monitoring process, bounce handling, unsubscribe suppression, owner assignment, and stop controls using mail sinks. Begin with human-triggered exact-message sends only after the legal sender identity, verified outbound provider/domain, and reply coverage are ready. A future Resend adapter must set an explicitly verified `From`, the Cloudflare-routed owner `Reply-To`, a stable `Message-ID`, and `In-Reply-To`/`References` for programmatic replies; persist provider IDs and reconcile webhook events idempotently. Cloudflare forwarding cannot itself send from the custom domain: M4 must prove the app's human-triggered Resend reply flow or a separately reviewed free owner-only send-as route. No automatic follow-ups.
...
The total ceiling remains C$50/month, excluding the existing ChatGPT/Codex subscription. It covers actual allocated engine costs, including platform, domain allocation, source/verification/AI, tax and FX. No account, mailbox, paid commitment, or free-tier entitlement is presumed active. The target first route has zero paid mailbox seats: Cloudflare Email Routing forwards inbound mail to existing owner inboxes, while Resend Free is a conditional outbound option whose current published limits and account eligibility require owner verification. If Cloudflare forwarding plus the gated Resend Free or separately reviewed free owner-only send-as route cannot meet reply ownership or privacy requirements, mail activation remains blocked. Paid mailboxes and Workspace seats are out of scope unless Riley explicitly reverses the zero-paid-mailbox decision.
...
| M4 — Ready-to-contact workbench and reply readiness | Exact messages/manual tasks with contact/consent proof and a named reply owner | Cloudflare destination/routing proof, Resend outbound/reply capability gates or an approved manual owner route, suppression, sink-tested replies, approval and stop controls; still no live send |
...
The plan selects architecture and sequencing now. Before actions become real, obtain concrete review of: actual provider/account commitments and taxes; approved sender/legal identification and mailing address; Cloudflare destination ownership/rules; Resend domain/key/webhook state or the reviewed owner-only send-as route; source-data rights and exact research scope; ten-business manifest; each deployed release/migration; any production policy experiment; exact outreach campaign/cohort; and a budget increase if the measured setup cannot fit. Public DNS is evidence only: it cannot prove forwarding activation, Resend account/key access, sender verification, complete DKIM, or send readiness.
