...
## LOCAL ONLY 2026-09-25: connected-calling foundation verification

Immutable local candidate `3233ff8879e992be0aa141ed61e8b4f9c9378433` passed safety, all 940 tests, typecheck and lint (0 errors, 4 existing warnings). Cloudflare packaging stopped at the existing private-file guard: Next 16.3.6 constructed Windows exclusion globs with backslashes and copied six synthetic M2 test-evidence files into the server bundle. A narrow patch normalizes both exclusion patterns and candidate paths; an installed-package regression reproduces the failure with Windows paths and checks POSIX too. The patch does not weaken the bundle guard. Full build/dry-run verification of the corrected tree remains pending.

Caller is checkpointed at `dadfa82` (242 checks passing and offline real-IndexedDB browser acceptance); Orbit is checkpointed at `44ac6fa` (2,231 tests, typecheck and build). Cross-system link implementation is in progress and is not part of that candidate. All new migrations remain unapplied; no production, automation, provider spend, installed extension or outreach changed. Next: verify the packaging fix; finish linked ownership/projections; complete reviewed decisions, conversion, recovery and device/release gates from the approved plan.
