...
**Reuse:** current contact-discovery/verification/persistence modules, owner read models, the legacy Gmail behavior/tests only for read-only reconciliation, `src/lib/outreach-approval.ts`, consent/suppression data, existing auth, and the typed provider-adapter boundary. The selected zero-paid-mailbox target is Cloudflare Email Routing from the root domain to verified existing owner destinations for inbound mail only. No cold-email sender is selected: Resend's current AUP prohibits unsolicited messages, including cold outreach, and Cloudflare Email Service currently describes its use as transactional-only. Resend may be reviewed separately for explicit opt-in or transactional mail if the current terms allow the exact use. Public DNS queried 2026-09-23 confirms Cloudflare root MX/SPF, DMARC quarantine, a Resend verification token, two public values at `resend._domainkey`, and SES-looking `send` SPF/MX records; it does not prove active forwarding, account/domain verification, which DKIM value is current, or permission/readiness to send. Characterize legacy behavior before adapting it; do not route new v2 records through legacy autonomous queues as a shortcut. ([Resend AUP](https://resend.com/legal/acceptable-use), [Cloudflare Email Service FAQ](https://developers.cloudflare.com/email-service/reference/faq/))
...
5. Establish the documented manual Cloudflare-forwarded reply, bounce, and suppression procedure first. Do not implement cold-email sending through Resend or Cloudflare Email Service under their current published policies. Manual calls and other separately approved owner tasks remain the available first-touch work. If a compatible provider/legal route is later proven, require an owner decision before selecting it. For opt-in or transactional Resend use only, a future adapter would need a verified `From`, Cloudflare-routed owner `Reply-To`, stable message/thread headers, signed webhook validation, `svix-id` idempotency, provider-state reconciliation, and immediate bounce/complaint/suppression handling. Legacy Gmail OAuth/send/inbox-sync routes remain quarantined from v2; Gmail incremental sync is a separately reviewed fallback, not the target architecture. Cloudflare forwarding itself is inbound only.
...
**Exit:** sender identity for manual channels, legal contact details, verified Cloudflare destination/routing ownership, exact approvals, source/consent/verification, suppressions, reply owner and stop controls all demonstrable. Any opt-in/transactional email capability must also have an owner-verified account and policy-permitted provider route, verified domain/key/webhook state and sink-tested reply handling. No provider route for cold email is assumed. Any OAuth scope/verification exceptions are assessed only for a separately approved legacy path. Still no live send.

## M5 — Controlled first-touch experiment

Prepare one exact cohort/campaign packet with owners, offer, source/consent evidence, channel, sender, message digests, limits, timing, reply coverage, cost and stop conditions. Obtain the distinct real-outreach authorization. Until a compatible email provider and legal route are proven, do not use email for cold first touch; prioritize owner-managed calls and other separately approved manual tasks. The proposed 5–10 total first touches/week is not permission to use a disallowed channel. No automatic follow-ups, simultaneous multi-contact approach, or automated calls/forms/DMs.
