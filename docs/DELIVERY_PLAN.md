...
**Reuse:** current contact-discovery/verification/persistence modules, owner read models, legacy Gmail behavior and tests, `src/lib/outreach-approval.ts`, consent/suppression data, existing auth. Characterize legacy behavior before adapting it; do not route new v2 records through legacy autonomous queues as a shortcut.
...
5. Establish either the documented manual reply/bounce/suppression procedure for human-triggered pilot mail, or authorized Gmail incremental sync. Production app-triggered sending requires demonstrated fresh reply/suppression handling. For two mailboxes, polling is simpler than Pub/Sub unless measurements justify push.
...
**Exit:** sender identity, legal contact details, verified mailboxes/domain authentication/recovery, exact approvals, source/consent/verification, suppressions, reply owner and stop controls all demonstrable. OAuth scope/verification exceptions assessed for the actual organization. Still no live send.
