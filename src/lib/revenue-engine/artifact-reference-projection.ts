...
export const ArtifactEvidenceUseEndRecordSchema = ArtifactEvidenceUseEndCoreSchema.extend({
  ...
  const replacement = record.reasonCode === "REPLACED_BY_EVIDENCE_USE";
  if (replacement !== (record.basis.basisType === "EVIDENCE_USE_REPLACEMENT" && record.replacementEvidenceUseId !== null)) {
    context.addIssue({ code: "custom", message: "Replacement ending fields must agree.", path: ["replacementEvidenceUseId"] });
  }
...
export const ArtifactManifestAvailabilitySchema = z.object({
  manifestId: z.string().uuid(),
...
export const ArtifactReferenceSnapshotReceiptSchema = z.object({
  ...
  sourceKind: z.literal("FIXTURE_COMPLETE_SNAPSHOT"),
...
function uniqueUses(facts: ArtifactReferenceSourceFacts) {
  ...
  for (const link of facts.manifestEvidenceUses) {
    const existing = uses.get(link.evidenceUse.useId);
    if (existing && !exactEqual(existing, link.evidenceUse)) throw new Error(`Evidence use ${link.evidenceUse.useId} has conflicting content.`);
    uses.set(link.evidenceUse.useId, link.evidenceUse);
  }
...
export function createFixtureArtifactReferenceSnapshotReceipt(value: {
  ...
  return ArtifactReferenceSnapshotReceiptSchema.parse({
    ...
    sourceCounts: {
      ...
      evidenceUses: uniqueUses(facts).length,
...
export const ArtifactReferenceProjectionSchema = ArtifactReferenceProjectionCoreSchema.extend({
  ...
  const expectedCounts = {
    ...
    evidenceUses: sourceUses.length,
  ...
  const expectedState = ambiguous.length || unassigned.length
    ? "INDETERMINATE"
    : active.length === 0
      ...
      : expectedRequired === "LEGAL_HOLD"
        ? "LEGAL_HOLD_ACTIVE"
        : "ACTIVE_REFERENCES";
  if (projection.state !== expectedState || projection.retentionReviewSuggested !== (expectedState === "NO_CURRENT_REFERENCES")) {
...
function requireUniqueBy<T>(values: T[], keyOf: (value: T) => string, label: string) {
  ...
  for (const value of values) {
    ...
    if (existing) throw new Error(`${label} ${key} appears more than once in a complete snapshot.`);
...
function validateSnapshot(facts: ArtifactReferenceSourceFacts, snapshot: ArtifactReferenceSnapshotReceipt) {
  ...
  if (!exactEqual(expected, snapshot)) throw new Error("Reference snapshot counts or source digest do not match its complete fact set.");
}
...
function manifestAvailable(
  ...
  if (["SHADOW_30D", "QUALIFICATION_180D"].includes(manifest.retentionClass)) {
    return availability.expiresAt !== null && Date.parse(availability.expiresAt) > Date.parse(projectedAt);
  }
...
export function createArtifactReferenceProjection(value: unknown): ArtifactReferenceProjection {
  ...
  if (Date.parse(request.projectedAt) < Date.parse(request.snapshot.snapshotCapturedAt)) {
    throw new Error("A reference projection cannot predate its complete snapshot.");
  }
  ...
  if (!root) throw new Error("Lineage root manifest is absent from the complete snapshot.");
  ...
  for (const ending of ends.values()) {
    ...
    if (Date.parse(ending.recordedAt) > Date.parse(request.snapshot.snapshotCapturedAt)) {
      throw new Error("Evidence-use ending cannot postdate the complete snapshot.");
    }
  }
  ...
  for (const use of uses) {
    ...
    const candidates = [...links.values()]
      ...
      .filter((manifest) => manifestAvailable(manifest, availability.get(manifest.manifestId)!, request.projectedAt));
  ...
  const requiredRetentionClass = activeUses.length
    ...
    : null;
  const state = ambiguousUseCount || unassignedUseCount
    ? "INDETERMINATE"
    : activeUses.length === 0
      ? "NO_CURRENT_REFERENCES"
  ...
  return ArtifactReferenceProjectionSchema.parse({
    ...
    sourceFacts: facts,
    uses: projectedUses,
    retentionReviewSuggested: state === "NO_CURRENT_REFERENCES",
...
export function verifyArtifactReferenceProjectionCurrent(
  ...
  const state = Date.parse(checkedAt) > Date.parse(projection.freshUntil) || currentSourceFactsDigest !== projection.sourceFactsDigest
    ? "STALE"
    : Date.parse(checkedAt) < Date.parse(projection.snapshotCapturedAt)
      ? "CONFLICT"
      : "CURRENT";
  return {
    state,
    sourceFactsMatch: currentSourceFactsDigest === projection.sourceFactsDigest,
