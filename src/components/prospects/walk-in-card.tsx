  ...
  return <li className="rounded-xl border border-slate-200 bg-white p-4">
    <div className="flex items-start justify-between gap-3">
      ...
      <div className="flex shrink-0 flex-col items-end gap-1 text-xs">
        <a href={stop.mapsUrl} target="_blank" rel="noreferrer noopener" className="inline-flex items-center gap-1 font-medium text-emerald-800 hover:underline"><MapPin className="size-3.5" aria-hidden="true" />Map</a>
    ...
    <button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open}
      className="mt-3 rounded-md bg-emerald-700 px-3 py-1.5 text-sm font-semibold text-white hover:bg-emerald-800">{open ? "Close" : "Log visit"}</button>
