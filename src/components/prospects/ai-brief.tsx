...
export function AiBrief({ prospectId, initial, aiReady }: { prospectId: string; initial: Brief | null; aiReady: boolean }) {
  ...
  return <div className="rounded-2xl border border-violet-200 bg-gradient-to-b from-violet-50 to-white p-5">
    <div className="flex items-center justify-between gap-2">
      <h3 className="flex items-center gap-2 text-sm font-semibold text-violet-900"><Sparkles className="size-4" aria-hidden="true" />AI call brief</h3>
      {state.status === "ready" ? <button type="button" onClick={() => void load()} className="inline-flex items-center gap-1 text-xs font-medium text-violet-800 hover:underline"><RefreshCw className="size-3" aria-hidden="true" />Refresh</button> : null}
    </div>
    {state.status === "idle" ? <div className="mt-3">
      ...
      <button type="button" onClick={() => void load()} disabled={!aiReady} className="mt-3 rounded-lg bg-violet-700 px-3 py-2 text-sm font-semibold text-white hover:bg-violet-800 disabled:cursor-not-allowed disabled:opacity-50">Write my brief</button>
    ...
    {state.status === "loading" ? <div className="mt-3 space-y-2" aria-live="polite">
      ...
      <div className="h-3 w-3/4 animate-pulse rounded bg-violet-100" /><div className="h-3 w-2/3 animate-pulse rounded bg-violet-100" /><div className="h-3 w-1/2 animate-pulse rounded bg-violet-100" />
    </div> : null}
    {state.status === "error" ? <div className="mt-3"><p role="alert" className="text-sm text-rose-700">{state.message}</p>
      {!state.setup ? <button type="button" onClick={() => void load()} className="mt-2 text-sm font-medium text-violet-800 hover:underline">Try again</button> : null}</div> : null}
    {state.status === "ready" ? <div className="mt-3 space-y-4 text-sm">
      <div><p className="text-xs font-semibold uppercase tracking-wide text-violet-900">Say this</p><p className="mt-1 rounded-lg bg-white p-3 leading-relaxed ring-1 ring-violet-100">{state.brief.opener}</p></div>
      <div><p className="text-xs font-semibold uppercase tracking-wide text-violet-900">Points to make</p>
        <ul className="mt-1 list-disc space-y-1 pl-5">{state.brief.talkingPoints.map((point) => <li key={point}>{point}</li>)}</ul></div>
      <div><p className="text-xs font-semibold uppercase tracking-wide text-violet-900">If they say…</p>
        <ul className="mt-1 space-y-2">{state.brief.objections.map((item) => <li key={item.objection} className="rounded-lg bg-white p-2.5 ring-1 ring-violet-100">
          <p className="flex items-start gap-1.5 font-medium"><MessageCircleQuestion className="mt-0.5 size-4 shrink-0 text-violet-700" aria-hidden="true" />&ldquo;{item.objection}&rdquo;</p>
