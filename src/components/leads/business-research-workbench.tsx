...
function VerifiedWorkbench({ result }: { result: Verified }) {
  ...
  return <div className="mx-auto min-h-[calc(100svh-6rem)] max-w-[1580px] px-4 pb-28 pt-4 text-[#1a3124] sm:px-6 sm:pt-6 md:pb-8 xl:px-8">
    ...
    <div className={`${mobileDetailOpen ? "mt-0 md:mt-5" : "mt-4 md:mt-5"} grid items-start gap-3 md:grid-cols-[minmax(270px,0.36fr)_minmax(0,0.64fr)] xl:gap-4`}>
      <section className={`${mobileDetailOpen ? "hidden md:block" : "block"} overflow-hidden rounded-2xl border border-[#e2e8e1] bg-white`} aria-label="Business queue">
        <div className="border-b border-[#edf0eb] p-3.5 sm:p-4"><div className="flex items-baseline justify-between gap-2"><h2 className="text-base font-semibold tracking-tight text-[#243a2b]">Businesses</h2><span className="text-xs font-semibold text-[#53645b]">{result.entries.length} saved</span></div><label className="relative mt-2.5 block"><Search className="pointer-events-none absolute left-3 top-3 size-4 text-[#53645b]" aria-hidden="true" /><span className="sr-only">Find a business</span><input type="search" value={query} onChange={(event) => { setQuery(event.target.value); setMobileDetailOpen(false); }} placeholder="Find a business" className="h-10 w-full rounded-lg border border-[#e2e9df] bg-white pl-9 pr-3 text-sm text-[#243c2d] placeholder:text-[#53645b] focus:border-[#4d9970] focus:outline-none focus:ring-2 focus:ring-[#a7d5b8]" /></label><div role="group" className="mt-2.5 flex flex-wrap gap-1" aria-label="Filter businesses">{filterOptions.map((option) => <button key={option.id} type="button" aria-pressed={filter === option.id} onClick={() => { setFilter(option.id); setMobileDetailOpen(false); }} className={`rounded-lg px-2.5 py-1.5 text-xs font-semibold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#196b50] ${filter === option.id ? "bg-[#174e37] text-white" : "text-[#66786b] hover:bg-[#f1f5ef]"}`}>{option.label} <span className={filter === option.id ? "text-white/70" : "text-[#53645b]"}>{countFor(option.id)}</span></button>)}</div></div>
