...
function BusinessDetail({ entry, verifiedAt, onBack, headingRef }: {
  ...
  return <section className="min-w-0 rounded-2xl border border-[#e2e8e1] bg-white" aria-label={`${entry.businessName} website review`}>
    <div className="border-b border-[#edf0eb] px-4 py-4 sm:px-6 sm:py-5">
      ...
      {websiteUrl ? <a href={websiteUrl} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex min-h-10 items-center gap-2 rounded-lg bg-[#145c46] px-4 text-sm font-semibold text-white transition hover:bg-[#0e4936] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#145c46]">Open website <ExternalLink className="size-4" aria-hidden="true" /></a> : null}
    </div>

    <div className="px-4 py-4 sm:px-6 sm:py-5">
      <div className="rounded-xl border border-[#e5e9e3] bg-white p-4 sm:p-5">
      ...
      <div className="mt-6">
        ...
        {pages.length > 0 ? <ul className="mt-4 space-y-2" aria-label={`${entry.businessName} pages checked`}>
          {pages.map((page) => {
            ...
            return <li key={`${page.pageKind}-${page.requestedUrl}`} className="rounded-xl border border-[#e5ebe4] bg-white px-4 py-3">
      ...
      <div className="mt-6 grid gap-2 sm:grid-cols-3">
        <div className="rounded-xl border border-[#e5e9e3] bg-white p-3.5"><p className="text-xs font-semibold text-[#53645b]">Website fit</p><p className="mt-1 text-sm font-semibold text-[#273b2e]">Not assessed</p></div>
        <div className="rounded-xl border border-[#e5e9e3] bg-white p-3.5"><p className="text-xs font-semibold text-[#53645b]">Mobile experience</p><p className="mt-1 text-sm font-semibold text-[#273b2e]">Not checked</p></div>
        <div className="rounded-xl border border-[#e5e9e3] bg-white p-3.5"><p className="text-xs font-semibold text-[#53645b]">Contact readiness</p><p className="mt-1 text-sm font-semibold text-[#273b2e]">Not reviewed</p></div>
      </div>
...
function VerifiedWorkbench({ result }: { result: Verified }) {
  ...
  return <div className="mx-auto min-h-[calc(100svh-6rem)] max-w-[1580px] px-4 pb-[calc(9rem+env(safe-area-inset-bottom))] pt-4 text-[#1a3124] sm:px-6 sm:pt-6 md:pb-8 xl:px-8">
    <header className={`${mobileDetailOpen ? "sr-only md:not-sr-only" : ""} px-1 sm:px-0`}>
      ...
      <h1 className="mt-2 text-[30px] font-semibold leading-tight tracking-[-0.04em] text-[#14291d] sm:text-[36px]">Business review</h1>
      <p className="mt-1.5 max-w-2xl text-sm leading-5 text-[#607267]">Explore the website research for each business. Website fit has not been decided, and this page cannot contact anyone.</p>
    </header>
    ...
    <div className={`${mobileDetailOpen ? "mt-0 md:mt-5" : "mt-4 md:mt-5"} grid items-start gap-3 md:grid-cols-[minmax(270px,0.36fr)_minmax(0,0.64fr)] xl:gap-4`}>
      <section className={`${mobileDetailOpen ? "hidden md:block" : "block"} overflow-hidden rounded-2xl border border-[#e2e8e1] bg-white`} aria-label="Business queue">
        <div className="border-b border-[#edf0eb] p-3.5 sm:p-4"><div className="flex items-baseline justify-between gap-2"><h2 className="text-base font-semibold tracking-tight text-[#243a2b]">Businesses</h2><span className="text-xs font-semibold text-[#53645b]">{result.entries.length} saved</span></div><label className="relative mt-2.5 block"><Search className="pointer-events-none absolute left-3 top-3 size-4 text-[#53645b]" aria-hidden="true" /><span className="sr-only">Find a business</span><input type="search" value={query} onChange={(event) => { setQuery(event.target.value); setMobileDetailOpen(false); }} placeholder="Find a business" className="h-10 w-full rounded-lg border border-[#e2e9df] bg-white pl-9 pr-3 text-sm text-[#243c2d] placeholder:text-[#53645b] focus:border-[#4d9970] focus:outline-none focus:ring-2 focus:ring-[#a7d5b8]" /></label><div role="group" className="mt-2.5 flex flex-nowrap gap-1 overflow-x-auto md:flex-wrap md:overflow-visible" aria-label="Filter businesses">{filterOptions.map((option) => <button key={option.id} type="button" aria-pressed={filter === option.id} onClick={() => { setFilter(option.id); setMobileDetailOpen(false); }} className={`shrink-0 rounded-lg px-2.5 py-1.5 text-xs font-semibold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#196b50] ${filter === option.id ? "bg-[#174e37] text-white" : "text-[#66786b] hover:bg-[#f1f5ef]"}`}>{option.label} <span className={filter === option.id ? "text-white/70" : "text-[#53645b]"}>{countFor(option.id)}</span></button>)}</div></div>
