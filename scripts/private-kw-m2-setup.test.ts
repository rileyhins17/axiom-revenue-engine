...
const envelopePath = "data/kw-evaluation/m2-preflight-test.json";
const databasePath = path.join(root, "m2-preflight-database.sqlite");
const backupPath = path.join(root, "m2-preflight-backup.sqlite");
const receiptPath = path.join(root, "m2-preflight-receipt.json");
const quarantinePath = path.join(root, "m2-preflight-quarantine.sqlite");
...
function envelope(overrides: Record<string, unknown> = {}) {
  const core = {
    ...
    databasePath: "data/kw-evaluation/m2-preflight-database.sqlite", backupPath: "data/kw-evaluation/m2-preflight-backup.sqlite", receiptPath: "data/kw-evaluation/m2-preflight-receipt.json", quarantinePath: "data/kw-evaluation/m2-preflight-quarantine.sqlite", localSchemaMutationAuthorized: true, remoteMigrationAuthorized: false, providerOperationsAuthorized: 0, runtimeQualificationAuthorized: false, runtimeContactAuthorized: false, runtimeOutreachAuthorized: false, runtimeSendAuthorized: false, deploymentAuthorized: false, costAuthorizedUsd: 0, rollbackPolicy: "QUARANTINE_AND_OWNER_APPROVED_RESTORE", approvedBy: "RILEY", reviewedAt: "2026-09-21T10:00:00.000Z", expiresAt: "2099-09-22T10:00:00.000Z", rationale: "Bounded local preflight fixture only.", confirmation: "APPROVE_M2_LOCAL_0069_SETUP",
    ...overrides,
  };
...
async function prepare() {
  ...
  await writeFile(databasePath, Buffer.from("synthetic preflight database"));
  await writeFile(envelopePath, JSON.stringify(envelope()));
  await rm(backupPath, { force: true }); await rm(receiptPath, { force: true }); await rm(quarantinePath, { force: true }); await rm(PRIVATE_KW_M2_SETUP_LOCK_PATH, { force: true });
}
...
describe("private KW M2 setup preflight", () => {
  ...
  it("fails closed for missing, invalid, expired, colliding, and symlink targets", async () => {
    ...
    try {
      ...
      await writeFile(envelopePath, JSON.stringify(envelope({ backupPath: "data/kw-evaluation/m2-preflight-database.sqlite" }))); await assert.rejects(preflightPrivateKwM2Setup(envelopePath), /distinct/);
      await writeFile(envelopePath, JSON.stringify(envelope()));
