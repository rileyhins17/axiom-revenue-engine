...
const privateKwM2CaptureCli = await readFile(new URL("./capture-private-kw-m2-html.ts", import.meta.url), "utf8");
...
requireMatch("package.json", packageJson, /"kw:capture-m2-html"\s*:\s*"tsx scripts\/capture-private-kw-m2-html\.ts"/, "M2 capture must use the bounded one-request operator command");
requireMatch("scripts/capture-private-kw-m2-html.ts", privateKwM2CaptureCli, /executePrivateKwM2HtmlEvidence\(/, "M2 capture must use the existing approval-gated HTML workflow");
requireMatch("scripts/capture-private-kw-m2-html.ts", privateKwM2CaptureCli, /reloadPrivateKwM2WebsiteEvidenceReceipt\(/, "M2 verification must reload saved evidence directly");
forbidMatch("scripts/capture-private-kw-m2-html.ts", privateKwM2CaptureCli, /--remote|--approve|--force|--clock|--provider|--database|--url|--business-id|--root|better-sqlite3|D1Database|R2Bucket|fetch\s*\(|process\.env/i, "M2 capture must not add approval bypasses, arbitrary targets, database/provider controls, or alternate network paths");
...
requireMatch("src/lib/revenue-engine/owner-lead-read-model.ts", ownerLeadReadModel, /function ownerLeadCandidateQuery\(partition: RevenueEvidencePartition\)[\s\S]*?return `\s*SELECT/, "owner lead D1 access must begin from an explicit SELECT contract");
requireMatch("src/lib/revenue-engine/owner-lead-read-model.ts", ownerLeadReadModel, /const partition = await readRevenueEvidencePartition\(database\)/, "owner lead list must inspect evidence partitions before querying");
requireMatch("src/lib/revenue-engine/owner-lead-read-model.ts", ownerLeadReadModel, /\.prepare\(ownerLeadCandidateQuery\(partition\)\)/, "owner lead list must use its inspected evidence partition");
...
requireMatch("src/lib/revenue-engine/owner-lead-detail-read-model.ts", ownerLeadDetailReadModel, /function ownerLeadDetailQuery\(partition: RevenueEvidencePartition\)[\s\S]*?return `\s*SELECT/, "owner lead detail D1 access must begin from an explicit exact SELECT contract");
requireMatch("src/lib/revenue-engine/owner-lead-detail-read-model.ts", ownerLeadDetailReadModel, /function ownerLeadContactReviewQuery\(partition: RevenueEvidencePartition\)[\s\S]*?return `\s*SELECT/, "owner contact-review visibility must use an explicit read-only SELECT contract");
requireMatch("src/lib/revenue-engine/owner-lead-detail-read-model.ts", ownerLeadDetailReadModel, /const partition = await readRevenueEvidencePartition\(database\)/, "owner lead detail must inspect evidence partitions before querying");
requireMatch("src/lib/revenue-engine/owner-lead-detail-read-model.ts", ownerLeadDetailReadModel, /\.prepare\(ownerLeadDetailQuery\(partition\)\)/, "owner lead detail must use its inspected evidence partition");
