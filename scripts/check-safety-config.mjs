...
const privateKwOwnerAuthIdempotencyRecovery = await readFile(new URL("../src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery.ts", import.meta.url), "utf8");
const privateKwOwnerAuthIdempotencyRecoveryProof = await readFile(new URL("../src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts", import.meta.url), "utf8");
const privateKwOwnerAuthDossierAcceptD1Plan = await readFile(new URL("../src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts", import.meta.url), "utf8");
const privateKwOwnerAuthDossierAcceptD1Executor = await readFile(new URL("../src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts", import.meta.url), "utf8");
...
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery.ts", privateKwOwnerAuthIdempotencyRecovery, /PRIVATE_KW_OWNER_AUTH_IDEMPOTENCY_RECOVERY_VERSION\s*=\s*\n?\s*"kw-owner-auth-idempotency-recovery-v1"/, "owner-auth recovery must use the reviewed versioned contract");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery.ts", privateKwOwnerAuthIdempotencyRecovery, /PRIVATE_KW_OWNER_AUTH_RESERVATION_EXPECTED_MAX_AGE_MS\s*=\s*120_000/, "owner-auth recovery must use the bounded reservation age");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery.ts", privateKwOwnerAuthIdempotencyRecovery, /PRIVATE_KW_OWNER_AUTH_MUTATION_CLAIM_PREDICATE[\s\S]*?state[^\n]*RESERVED[\s\S]*?payloadDigest/, "owner-auth recovery must define the exact parameterized reservation claim");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery.ts", privateKwOwnerAuthIdempotencyRecovery, /visibleReservationAction:\s*z\.literal\("BLOCK_AND_ESCALATE"\)[\s\S]*?reservationMayBeFinalizedByRecovery:\s*z\.literal\(false\)/, "visible and stale reservations must fail closed without recovery mutation");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery.ts", privateKwOwnerAuthIdempotencyRecovery, /sameKeyRetryAfterReservation:\s*z\.literal\("FAIL_CLOSED_UNTIL_NEW_KEY"\)/, "a visible reservation must not be reused under the same key");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery.ts", privateKwOwnerAuthIdempotencyRecovery, /claimPredicate:\s*z\.literal\(PRIVATE_KW_OWNER_AUTH_MUTATION_CLAIM_EXISTS_PREDICATE\)[\s\S]*?executableByThisContract:\s*z\.literal\(false\)/, "operation claims must embed the exact non-executable reservation predicate");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery.ts", privateKwOwnerAuthIdempotencyRecovery, /operationMutationAllowed:\s*z\.literal\(false\)[\s\S]*?reservationRecoveryMutationAllowed:\s*z\.literal\(false\)/, "recovery decisions must authorize neither recovery nor operation mutation");
for (const field of ["visibleReservationRecoveryAuthorized", "reservationReclaimAuthorized", "operationMutationAuthorized", "ownerDecisionPersistenceAuthorized", "phaseInputCreationAuthorized", "progressReceiptCreationAuthorized", "phaseAdvancementAuthorized", "databaseReadAuthorized", "databaseMutationAuthorized", "routeAuthorized", "uiMutationAuthorized", "outreachAuthorized", "sendAuthorized", "deploymentAuthorized"]) {
  requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery.ts", privateKwOwnerAuthIdempotencyRecovery, new RegExp(`${field}:\\s*z\\.literal\\(false\\)`), `${field} must remain false in owner-auth recovery`);
}
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery.ts", privateKwOwnerAuthIdempotencyRecovery, /providerOperationsAuthorized:\s*z\.literal\(0\)[\s\S]*?costAuthorizedUsd:\s*z\.literal\(0\)/, "owner-auth recovery must authorize zero provider operations and cost");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery.ts", privateKwOwnerAuthIdempotencyRecovery, /const trustedObservations = new WeakSet<object>\(\)[\s\S]*?trustedObservations\.has\(value\)[\s\S]*?trustedDecisions\.add\(decision\)/, "owner-auth recovery consumers must require exact in-process observation and decision values");
forbidMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery.ts", privateKwOwnerAuthIdempotencyRecovery, /@cloudflare|@\/lib\/auth|better-auth|env\.[A-Z_]+|D1Database|R2Bucket|node:fs|readFile|writeFile|fetch\s*\(|\.prepare\s*\(|\.batch\s*\(|\.run\s*\(|\.put\s*\(|\.delete\s*\(/, "owner-auth recovery must stay disconnected from auth adapters, runtime, files, live databases, providers, and network");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts", privateKwOwnerAuthIdempotencyRecoveryProof, /PRIVATE_KW_OWNER_AUTH_RECOVERY_PROOF_VERSION\s*=\s*\n?\s*"kw-owner-auth-recovery-proof-v1"/, "owner-auth recovery proof must use the reviewed versioned contract");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts", privateKwOwnerAuthIdempotencyRecoveryProof, /assurance:\s*z\.literal\("CALLER_COUNTS_VALIDATED_ONLY"\)[\s\S]*?durableExecutionProven:\s*z\.literal\(false\)/, "caller count validation must never claim durable transaction provenance");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts", privateKwOwnerAuthIdempotencyRecoveryProof, /assurance:\s*z\.literal\("RECOVERY_CANDIDATE_ONLY"\)[\s\S]*?durableExecutionProven:\s*z\.literal\(false\)/, "abandonment receipts must remain recovery candidates rather than durable execution proof");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts", privateKwOwnerAuthIdempotencyRecoveryProof, /Number\.isFinite\(Date\.parse\(value\)\)/, "recovery timestamps must resolve to finite epochs before chronology comparisons");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts", privateKwOwnerAuthIdempotencyRecoveryProof, /rowCountScope:\s*z\.literal\("EXACT_IDEMPOTENCY_KEY"\)/, "affected-row proofs must be scoped to the exact idempotency key");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts", privateKwOwnerAuthIdempotencyRecoveryProof, /PRIVATE_KW_OWNER_AUTH_RECOVERY_MIN_RETENTION_MS\s*=\s*365\s*\*\s*24\s*\*\s*60\s*\*\s*60\s*\*\s*1_000/, "owner-auth recovery proof must retain the proposed one-year minimum history");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts", privateKwOwnerAuthIdempotencyRecoveryProof, /recoveryState:\s*z\.literal\("ABANDONED"\)[\s\S]*?replacementKey:\s*z\.null\(\)[\s\S]*?sameKeyReuseAllowed:\s*z\.literal\(false\)/, "abandonment receipts must remain additive and cannot reuse or issue a key");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts", privateKwOwnerAuthIdempotencyRecoveryProof, /originalReservationPreserved:\s*z\.literal\(true\)[\s\S]*?recoveryReceiptAppendOnly:\s*z\.literal\(true\)[\s\S]*?deletionRequiresSeparateOwnerRelease:\s*z\.literal\(true\)/, "recovery retention must preserve the original and require a separate deletion release");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts", privateKwOwnerAuthIdempotencyRecoveryProof, /expectedOperationRows:\s*z\.literal\(1\)[\s\S]*?rollbackWholeBatch:\s*z\.literal\(true\)/, "affected-row proofs must require one operation row and whole-batch rollback");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts", privateKwOwnerAuthIdempotencyRecoveryProof, /FRESH_COMMIT[\s\S]*?operationRowsAffected !== 1[\s\S]*?idempotencyRowsInserted !== 1[\s\S]*?outboxRowsInserted !== 1/, "fresh mutation proofs must require exactly one operation, ledger, and outbox change");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts", privateKwOwnerAuthIdempotencyRecoveryProof, /EXACT_REPLAY[\s\S]*?operationRowsAffected !== 0[\s\S]*?outboxRowsInserted !== 0/, "exact replay proofs must require zero mutation and duplicate outbox changes");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts", privateKwOwnerAuthIdempotencyRecoveryProof, /ROLLBACK[\s\S]*?idempotencyRowsAfter !== 0[\s\S]*?outboxRowsAfter !== 0/, "rollback proofs must require zero durable ledger and outbox rows");
for (const field of ["abandonmentReceiptCreationAuthorized", "reservationReclaimAuthorized", "reservationDeleteAuthorized", "replacementKeyIssuanceAuthorized", "operationMutationAuthorized", "databaseReadAuthorized", "databaseMutationAuthorized", "routeAuthorized", "uiMutationAuthorized", "outreachAuthorized", "sendAuthorized", "deploymentAuthorized"]) {
  requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts", privateKwOwnerAuthIdempotencyRecoveryProof, new RegExp(`${field}:\\s*z\\.literal\\(false\\)`), `${field} must remain false in owner-auth recovery proof`);
}
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts", privateKwOwnerAuthIdempotencyRecoveryProof, /providerOperationsAuthorized:\s*z\.literal\(0\)[\s\S]*?costAuthorizedUsd:\s*z\.literal\(0\)/, "owner-auth recovery proof must authorize zero provider operations and cost");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts", privateKwOwnerAuthIdempotencyRecoveryProof, /const trustedRecoveryReceipts = new WeakSet<object>\(\)[\s\S]*?trustedRecoveryReceipts\.add\(receipt\)[\s\S]*?trustedRecoveryReceipts\.has\(value\)/, "recovery proof consumers must require exact in-process receipts");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts", privateKwOwnerAuthIdempotencyRecoveryProof, /const trustedMutationProofs = new WeakSet<object>\(\)[\s\S]*?trustedMutationProofs\.add\(proof\)[\s\S]*?trustedMutationProofs\.has\(value\)/, "affected-row proof consumers must require exact in-process proofs");
forbidMatch("src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts", privateKwOwnerAuthIdempotencyRecoveryProof, /@cloudflare|@\/lib\/auth|better-auth|env\.[A-Z_]+|D1Database|R2Bucket|node:fs|readFile|writeFile|fetch\s*\(|\.prepare\s*\(|\.batch\s*\(|\.run\s*\(|\.put\s*\(|\.delete\s*\(/, "owner-auth recovery proof must stay disconnected from auth adapters, runtime, files, live databases, providers, and network");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts", privateKwOwnerAuthDossierAcceptD1Plan, /PRIVATE_KW_OWNER_DOSSIER_ACCEPT_D1_PLAN_VERSION\s*=\s*\n?\s*"kw-owner-dossier-accept-d1-plan-v2"/, "the concrete owner dossier transaction must use the reviewed versioned contract");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts", privateKwOwnerAuthDossierAcceptD1Plan, /PRIVATE_KW_OWNER_DOSSIER_ACCEPT_OPERATION\s*=\s*\n?\s*"owner\.dossier\.accept"/, "the concrete transaction must target only owner dossier acceptance");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts", privateKwOwnerAuthDossierAcceptD1Plan, /requireInProcessPrivateKwOwnerAuthServerBoundary[\s\S]*?requireInProcessPrivateKwOwnerAuthIdempotencyCommitResult[\s\S]*?requireInProcessPrivateKwAuthenticatedOwnerDecisionRecord[\s\S]*?recheckPrivateKwAuthenticatedOwnerDecisionSessionBinding/, "the concrete transaction must bind the exact server boundary, replay result, owner decision, and current session");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts", privateKwOwnerAuthDossierAcceptD1Plan, /const claimExistsPredicate\s*=\s*\n?\s*PRIVATE_KW_OWNER_AUTH_MUTATION_CLAIM_EXISTS_PREDICATE[\s\S]*?INSERT INTO "RevenuePrivateKwOwnerDecision"[\s\S]*?WHERE \$\{claimExistsPredicate\}/, "the owner decision insert must embed the exact reservation claim");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts", privateKwOwnerAuthDossierAcceptD1Plan, /"reserve:owner_auth_idempotency"[\s\S]*?"mutation:owner_auth_operation"[\s\S]*?"finalize:owner_auth_idempotency"[\s\S]*?"insert:owner_auth_outbox"/, "the fresh transaction must reserve, mutate the exact decision, finalize, and enqueue in order");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts", privateKwOwnerAuthDossierAcceptD1Plan, /SET "state" = CASE WHEN changes\(\) = 1 THEN 'COMMITTED' ELSE NULL END/, "finalization must fail unless the immediately preceding owner-decision insert changed exactly one row in this transaction");
forbidMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts", privateKwOwnerAuthDossierAcceptD1Plan, /INSERT OR IGNORE/i, "fresh transaction conflicts must throw and roll back rather than become silent zero-row writes");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts", privateKwOwnerAuthDossierAcceptD1Plan, /SELECT "recordId" FROM "RevenuePrivateKwOwnerAuthIdempotency"[\s\S]*?"state" = 'COMMITTED'/, "outbox insertion must fail when the exact idempotency record did not commit");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts", privateKwOwnerAuthDossierAcceptD1Plan, /reservation:\s*z\.literal\(1\)[\s\S]*?ownerDecision:\s*z\.literal\(1\)[\s\S]*?finalization:\s*z\.literal\(1\)[\s\S]*?outbox:\s*z\.literal\(1\)/, "fresh transaction expectations must require exactly one change at every mutation step");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts", privateKwOwnerAuthDossierAcceptD1Plan, /replayBehavior:\s*z\.literal\("READ_EXACT_COMMITTED_BUNDLE_WITHOUT_MUTATION"\)/, "exact replay must remain a read-only bundle reload");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts", privateKwOwnerAuthDossierAcceptD1Plan, /SELECT "type", "name", "tbl_name",[\s\S]*?"schema_version"[\s\S]*?"tbl_name" IN \(\?, \?, \?\)/, "the preflight must read every trigger and schema version for all three guarded tables");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts", privateKwOwnerAuthDossierAcceptD1Plan, /freshSchemaVersionFenceBindings:\s*z\.literal\(2\)[\s\S]*?schema_version" FROM "pragma_schema_version"\) = \?/, "both fresh entry statements must require the exact preflight schema-version fence");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts", privateKwOwnerAuthDossierAcceptD1Plan, /materializePrivateKwOwnerDossierAcceptFreshStatements[\s\S]*?materializedBindings \+= 1[\s\S]*?materializedBindings !== plan\.freshSchemaVersionFenceBindings/, "schema-version materialization must replace and count the exact two trusted sentinels");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts", privateKwOwnerAuthDossierAcceptD1Plan, /planId:[\s\S]*?planDigest:[\s\S]*?artifactReferenceDigest\(core\)[\s\S]*?const trustedPlans = new WeakSet<object>/, "the concrete transaction plan must be content-addressed and exact-instance trusted");
for (const field of ["runtimeConnectionAuthorized", "migrationApplicationAuthorized", "ownerDecisionPersistenceAuthorized", "phaseInputCreationAuthorized", "progressReceiptCreationAuthorized", "phaseAdvancementAuthorized", "routeAuthorized", "uiMutationAuthorized", "outreachAuthorized", "sendAuthorized", "deploymentAuthorized"]) {
  requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts", privateKwOwnerAuthDossierAcceptD1Plan, new RegExp(`${field}:\\s*z\\.literal\\(false\\)`), `${field} must remain false in the concrete owner dossier transaction plan`);
}
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts", privateKwOwnerAuthDossierAcceptD1Plan, /providerOperationsAuthorized:\s*z\.literal\(0\)[\s\S]*?costAuthorizedUsd:\s*z\.literal\(0\)/, "the concrete transaction plan must authorize zero provider operations and cost");
forbidMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts", privateKwOwnerAuthDossierAcceptD1Plan, /@cloudflare|@\/lib\/auth|better-auth|env\.[A-Z_]+|D1Database|R2Bucket|node:fs|readFile|writeFile|fetch\s*\(|\.prepare\s*\(|\.batch\s*\(|\.run\s*\(|\.put\s*\(|\.delete\s*\(/, "the concrete owner dossier transaction plan must stay disconnected from auth adapters, runtime bindings, files, live databases, providers, and network");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts", privateKwOwnerAuthDossierAcceptD1Executor, /PRIVATE_KW_OWNER_DOSSIER_ACCEPT_D1_EXECUTOR_VERSION\s*=\s*\n?\s*"kw-owner-dossier-accept-d1-executor-v1"/, "the disconnected owner dossier executor must use the reviewed versioned contract");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts", privateKwOwnerAuthDossierAcceptD1Executor, /success:\s*z\.literal\(true\)[\s\S]*?changes:\s*z\.number\(\)\.int\(\)\.nonnegative\(\)/, "the executor must schema-validate D1 success and affected-row metadata");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts", privateKwOwnerAuthDossierAcceptD1Executor, /results:\s*z\.array\(SqlRowSchema\)\.max\(100\)\.nullable\(\)\.transform\(\(rows\) => rows \?\? \[\]\)/, "documented null D1 result arrays must normalize to empty arrays before exact count validation");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts", privateKwOwnerAuthDossierAcceptD1Executor, /PRIVATE_KW_OWNER_DOSSIER_ACCEPT_TRIGGER_DEFINITION_DIGESTS[\s\S]*?normalizePrivateKwOwnerAuthTriggerSql\(row\.sql\)[\s\S]*?actualDigest !== expectedDigest/, "the executor must verify the exact normalized body digest for all owner-auth triggers");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts", privateKwOwnerAuthDossierAcceptD1Executor, /type:\s*z\.literal\("trigger"\)[\s\S]*?tbl_name:\s*z\.enum[\s\S]*?row\.tbl_name !== expectedTable[\s\S]*?row\.schemaVersion !== schemaVersion/, "trigger preflight must bind exact object type, owning table, and one consistent schema version");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts", privateKwOwnerAuthDossierAcceptD1Executor, /unexpected target-table triggers are forbidden/, "extra triggers on guarded owner-action tables must fail closed");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts", privateKwOwnerAuthDossierAcceptD1Executor, /verifyPrivateKwAuthenticatedOwnerDecisionStoredBinding\([\s\S]*?plan\.expectedOwnerDecisionRecordJson[\s\S]*?await boundary\.batch\(plan\.preflightStatements\)/, "the executor must verify the planned signed decision before any database access");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts", privateKwOwnerAuthDossierAcceptD1Executor, /materializePrivateKwOwnerDossierAcceptFreshStatements\([\s\S]*?preflight\.schemaVersion[\s\S]*?await boundary\.batch\(freshStatements\)/, "the fresh batch must use the exact schema version observed by its preflight");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts", privateKwOwnerAuthDossierAcceptD1Executor, /expectedChanges = \[1, 1, 1, 1\][\s\S]*?result\.meta\.changes !== expected/, "fresh execution must require exactly one changed row for reservation, decision, finalization, and outbox");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts", privateKwOwnerAuthDossierAcceptD1Executor, /reservedAt > decisionRecordedAt[\s\S]*?decisionRecordedAt > committedAt[\s\S]*?committedAt !== recordedAt[\s\S]*?committedAt > outboxCreatedAt[\s\S]*?outboxCreatedAt > databaseNow/, "fresh and replay paths must enforce the complete durable bundle chronology");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts", privateKwOwnerAuthDossierAcceptD1Executor, /catch \(freshError\)[\s\S]*?await boundary\.batch\(plan\.replayStatements\)[\s\S]*?replay\.state !== "COMMITTED"/, "an ambiguous fresh error may recover only through a new exact read-only committed-bundle reload");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts", privateKwOwnerAuthDossierAcceptD1Executor, /const trustedExecutionResults = new WeakSet<object>\(\)[\s\S]*?trustedExecutionResults\.add\(trusted\)[\s\S]*?trustedExecutionResults\.has\(value\)/, "owner dossier execution consumers must require the exact in-process result");
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts", privateKwOwnerAuthDossierAcceptD1Executor, /liveD1ExecutionProven:\s*z\.literal\(false\)[\s\S]*?runtimeConnected:\s*z\.literal\(false\)[\s\S]*?migrationApplied:\s*z\.literal\(false\)/, "the disconnected executor must not claim live D1, runtime, or migration proof");
for (const field of ["runtimeConnectionAuthorized", "migrationApplicationAuthorized", "ownerDecisionPersistenceAuthorized", "phaseInputCreationAuthorized", "progressReceiptCreationAuthorized", "phaseAdvancementAuthorized", "routeAuthorized", "uiMutationAuthorized", "outreachAuthorized", "sendAuthorized", "deploymentAuthorized"]) {
  requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts", privateKwOwnerAuthDossierAcceptD1Executor, new RegExp(`${field}:\\s*z\\.literal\\(false\\)`), `${field} must remain false in the disconnected owner dossier executor`);
}
requireMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts", privateKwOwnerAuthDossierAcceptD1Executor, /providerOperationsAuthorized:\s*z\.literal\(0\)[\s\S]*?costAuthorizedUsd:\s*z\.literal\(0\)/, "the disconnected owner dossier executor must authorize zero provider operations and cost");
forbidMatch("src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts", privateKwOwnerAuthDossierAcceptD1Executor, /@cloudflare|@\/lib\/(?:auth|session)|better-auth|env\.[A-Z_]+|\bD1Database\b|R2Bucket|node:fs|readFile|writeFile|fetch\s*\(|\.run\s*\(|\.put\s*\(|\.delete\s*\(/, "the owner dossier executor must stay disconnected from auth adapters, files, named runtime bindings, providers, network, and direct mutation methods");
...
for (const [name, content] of [
  ...
  if (content.includes("private-kw-owner-auth-idempotency-recovery-proof")
    && ![
      "src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts",
      "src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.test.ts",
      "src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.test.ts",
      "src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts",
      "src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.test.ts",
      "scripts/check-safety-config.mjs",
    ].includes(name)) {
    failures.push(`${name}: recovery candidates and count validation must remain disconnected from runtime and operator scripts`);
  }
  if (content.includes("private-kw-owner-auth-dossier-accept-d1-plan")
    && ![
      "src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts",
      "src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.test.ts",
      "src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.ts",
      "src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.test.ts",
      "scripts/check-safety-config.mjs",
    ].includes(name)) {
    failures.push(`${name}: the concrete owner dossier transaction plan must remain unreachable from runtime, UI, routes, and operator scripts`);
  }
  if (
...
for (const [name, content] of [
  ...
  if (
    ...
    && name !== "src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery.ts"
    && name !== "src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery.test.ts"
    && name !== "src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.ts"
    && name !== "src/lib/revenue-engine/private-kw-owner-auth-idempotency-recovery-proof.test.ts"
    && name !== "src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.ts"
    && name !== "src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-plan.test.ts"
    && name !== "src/lib/revenue-engine/private-kw-owner-auth-dossier-accept-d1-executor.test.ts"
    && name !== "scripts/check-safety-config.mjs"
  ) {
